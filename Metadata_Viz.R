...
server <- function(input, output) {
  ...
  Completeness <- suppressWarnings(import("NSSP_MDWG_aYear.xlsx", which=6)) # Ignore warnings. These are triggered by odd entries (NR instead of %) but they are processed correctly (as NAs).
  ...
  Timeliness_Extended <- mutate(Timeliness, 
                                ...
                                County_Name = case_when(
                                  str_detect(Timeliness$Site_Name, ",") == 1 ~ str_split_fixed(Timeliness$Site_Name, ",", 2)[ , 1],
                                  str_detect(Timeliness$Site_Name, ",") == 0 ~ NA_character_ 
                                ),
  ...
  Completeness_Extended <- mutate(Completeness, 
                                  ...
                                  County_Name = case_when(
                                    str_detect(Completeness$Site_Name, ",") == 1 ~ str_split_fixed(Completeness$Site_Name, ",", 2)[ , 1],
                                    str_detect(Completeness$Site_Name, ",") == 0 ~ NA_character_ 
                                  ),
                                  pvisits = case_when(
                                    is.na(Completeness$pvisits) == TRUE ~ NA_real_, 
                                    is.na(Completeness$pvisits) == FALSE ~ (Completeness$pvisits * 100) %>% round(2)
                                  )
  )
