...
server <- function(input, output) {
  ...
  Timeliness_Fake <- readRDS("Timeliness_Fake.rds")
  Completeness_Fake <- readRDS("Completeness_Fake.rds")
  ...
  output$mymap <- renderLeaflet({
    ...
    if(input$domain == "completeness"){  
      ...
      state_data <- filter(Completeness_Fake, dqvar == input$field, Geographic_Level == "State") %>%
        rename(measure = pvisits)
      county_data <- filter(Completeness_Fake, dqvar == input$field, Geographic_Level == "County") %>%
        rename(measure = pvisits)
    }
    
    # If timeliness domain is selected  
    if (input$domain == "timeliness"){
      state_data <- filter(Timeliness_Fake, Geographic_Level == "State") %>%
        rename_(measure = input$measure)
      county_data <- filter(Timeliness_Fake, Geographic_Level == "County") %>%
        rename_(measure = input$measure)      
    } 
    ...
    if (input$domain == "completeness"){
      ...
      color_set <- colorBin("Reds", NULL, bins = c(0, 1, 2, 5, 10))
      state_popup <- paste0("<strong>State: </strong>", state_metadata_merged$NAME, 
